...
export class Workspace implements VizInfoFactory {
    ...
    public async initialize(): Promise<Workspace> {
        ...
        // TODO Map and assign.
        const _ignored = _.flatMap(meta.stationSensors.stations, (info, stationId) => {
            ...
            if (info.length > 0) {
                ...
                console.log(`viz:`, info, stationId);
                return [];
            }
        });
    ...
    */
    ...
    public sensorOptions(stationId: number, flatten = false, depth = 0): SensorTreeOption[] {
        ...
        const options = _.map(allModules, (sensors, moduleId: ModuleID): SensorTreeOption => {
            ...
            let label = i18n.tc(moduleKey);

            const station = this.stationsFull.find((s) => s.id === stationId);
            if (station) {
                const moduleWithLabel = _.flatten(station.configurations.all.map((config) => config.modules)).find(
                    (module) => module.hardwareIdBase64 === moduleId || module.hardwareId === moduleId
                );

                if (moduleWithLabel && moduleWithLabel.label) {
                    label = moduleWithLabel.label;
                }
            }
