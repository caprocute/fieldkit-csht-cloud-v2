...
abstract class MarkdownParser<T> implements md.NodeVisitor {
  ...
  void visitText(md.Text text) {
    ...
    // If we're in a list item, standardize bullet points
    if (last != null &&
        last is OkBuilder &&
        (last as OkBuilder).name == "list-item") {
      if (content.trim().startsWith(RegExp(r'^[•·*\-+](\s|$)'))) {
        content = content.replaceFirst(RegExp(r'^[•·*\-+](\s|$)'), "• ");
      }
    }

    // If we're in a link, trim whitespace to prevent new lines
    if (last != null &&
        last is OkBuilder &&
        (last as OkBuilder).name.startsWith("link")) {
...
class MarkdownVerifyParser extends MarkdownParser {
  ...
  @override
  Builder ordered() {
    return OkBuilder(name: "ordered", getChildren: true);
  }
}
