...
pub enum BackgroundMessage {
    ...
    StationReply(DeviceId, String, HttpReply, String),
}
...
impl NearbyDevices {
    ...
    async fn mark_retry(&self, device_id: &DeviceId) -> Result<Connection> {
        ...
        querying.failures += 1;
        if querying.busy {
            warn!(busy = true, failures = querying.failures, "{:?}", device_id)
        }
    ...
    async fn query_station(&self, querying: &Querying) -> Result<()> {
        ...
        let addr = querying.http_addr.clone();
        let status = client.query_readings(&addr).await?;
        Ok(self
            .mark_finished_and_publish_reply(&querying.device_id, addr, status)
            .await?)
    }
    ...
    pub async fn mark_busy(&self, device_id: &DeviceId, busy: bool) -> Result<()> {
        let mut devices = self.devices.lock().await;
        let querying = devices.get_mut(device_id).expect("Whoa, no querying yet?");
        if querying.busy != busy {
            debug!(busy = busy, "{:?}", device_id);
        }
        querying.busy = busy;
        Ok(())
    }
    ...
    pub async fn handle_server_event(
        ...
        match &event {
            ...
            ServerEvent::Transferring(device_id, first, started, progress) => {
