...
impl MergeAndPublishReplies {
    ...
    async fn handle_background_message(&self, bg: BackgroundMessage) -> Result<()> {
        match bg {
            ...
            BackgroundMessage::StationReply(device_id, addr, reply, raw) => {
                let station = {
                    let db = self.db.lock().await;
                    db.merge_reply(store::DeviceId(device_id.into()), reply.clone())?
                };
                ...
                Ok(self
                    ...
                    .send(DomainMessage::StationRefreshed(
                        ...
                        .try_into()?,
                        Some(EphemeralConfig::from_reply(reply, addr)?),
                        raw,
                    ))
                    .await?)
            }
...
impl Sdk {
    ...
    async fn start_download(
        ...
        Ok(TransferProgress {
            ...
            status: TransferStatus::Downloading(DownloadProgress {
                started: 0,
                completed: 0.0,
                first: first.map(|v| v as usize),
                received: first.unwrap_or(0) as usize,
    ...
    async fn start_upload(
        ...
        for file in files.iter() {
            match tokio::fs::metadata(&file.path).await {
                Ok(md) => {
                    total_bytes += md.len() as u64;
                }
                Err(e) => warn!("metadata error: {:?}", e),
            }

            total_readings += (file.tail - file.head) as u64;
        }
        ...
        tokio::task::spawn({
            ...
            async move {
                ...
                let mut readings_uploaded = 0;

                let mut success = match ourselves {
                    Err(e) => {
                        warn!("ourselves failed: {:?}", e);
                        false
                    }
                    Ok(_) => true,
                };
                            ...
                            Ok(mut stream) => {
                                ...
                                loop {
                                    match stream.next().await {
                                        Some(Ok(bytes)) => {
                                            match publish_tx
                                                .send(DomainMessage::UploadProgress(
                                                    TransferProgress {
                                ...
                                if failed {
                                    ...
                                    readings_uploaded += (file.tail - file.head) as u64;
    ...
    async fn configure(&self, device_id: DeviceId, config: impl Into<HttpQuery>) -> Result<()> {
        if let Some(addr) = self.get_nearby_addr(&device_id).await? {
            ...
            self.nearby
                .mark_finished_and_publish_reply(&device_id, addr, status)
                .await?;
        }
    ...
    async fn check_for_archives(&self) -> Result<()> {
        self.server.check_for_archives().await
    }

    async fn set_station_busy(&self, device_id: String, busy: bool) -> Result<()> {
        self.nearby.mark_busy(&DeviceId(device_id), busy).await
    }
    ...
    async fn forget_station(&self, device_id: DeviceId) -> Result<Vec<StationConfig>> {
        let connections = self.nearby.get_connections().await?;
        let db = self.db.lock().await;

        db.delete_station_by_device_id(&store::DeviceId(device_id.into()))?;
...
pub fn configure_schedule(device_id: String, config: ScheduleConfig) -> Result<()> {
    Ok(with_runtime(|rt, sdk| {
...
pub fn forget_station(device_id: String) -> Result<Vec<StationConfig>> {
    Ok(with_runtime(|rt, sdk| {
        rt.block_on(sdk.forget_station(DeviceId(device_id.clone())))
    })?)
}
...
pub fn start_download(
    ...
    Ok(with_runtime(|rt, sdk| {
        rt.block_on(sdk.start_download(DeviceId(device_id.clone()), first, total))
    })?)
}
...
pub fn set_station_busy(device_id: String, busy: bool) -> Result<()> {
    Ok(with_runtime(|rt, sdk| {
        rt.block_on(sdk.set_station_busy(device_id, busy))
    })?)
}

pub fn check_for_archives() -> Result<()> {
    Ok(with_runtime(|rt, sdk| {
        rt.block_on(sdk.check_for_archives())
    })?)
}
...
impl From<SimpleSchedule> for Schedule {
    fn from(value: SimpleSchedule) -> Self {
        match value {
            SimpleSchedule::Every(duration) => Self::Every(duration.as_secs() as u32),
        }
    }
}

impl From<SimpleSchedule> for ScheduleConfig {
    fn from(value: SimpleSchedule) -> Self {
        Self {
            schedule: value.into(),
        }
...
impl Into<HttpQuery> for ScheduleConfig {
    fn into(self) -> HttpQuery {
        let schedule: SimpleSchedule = self.schedule.into();
        let mut query = HttpQuery::default();
        query.r#type = QueryType::QueryConfigure as i32;
        query.schedules = Some(query::device::Schedules {
            modifying: true,
            readings: Some(schedule.into()),
            lora: None,
            network: None,
            gps: None,
        });
...
impl Into<HttpQuery> for DeployConfig {
    fn into(self) -> HttpQuery {
        ...
        query.recording = Some(Recording {
            modifying: true,
            enabled: self.deployed.is_some(),
            started_time: self.deployed.unwrap_or(0),
            location: None,
        });
        query.schedules = Some(query::device::Schedules {
            modifying: true,
...
impl Into<SimpleSchedule> for Schedule {
    fn into(self) -> SimpleSchedule {
        match self {
            Schedule::Every(seconds) => {
                SimpleSchedule::Every(std::time::Duration::from_secs(seconds as u64))
            }
        }
    }
}
...
pub struct SchedulesConfig {
    pub readings: Option<ScheduleConfig>,
}
...
impl EphemeralConfig {
    fn from_reply(value: HttpReply, addr: String) -> Result<EphemeralConfig, SdkMappingError> {
        ...
        let schedules = value.schedules.as_ref().map(|s| SchedulesConfig {
            readings: s.readings.as_ref().and_then(|r| {
                let maybe_simple: Result<SimpleSchedule, _> = r.clone().try_into();
                match maybe_simple {
                    Ok(s) => Some(ScheduleConfig::from(s)),
                    Err(e) => {
                        warn!("Schedule error: {:?}", e);
                        None
                    }
                }
            }),
        ...
        let lora = value.lora_settings.map(|ls| LoraConfig {
