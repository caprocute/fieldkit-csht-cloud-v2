...
class MarkdownWidgetParser extends MarkdownParser<Widget> {
  ...
  @override
  Builder<Widget, Widget> ordered() {
    return _OrderedBuilder();
  }
}
...
class MarkdownChildren extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (children.length == 1) {
      ...
      return Expanded(
        child: Column(
          // Left align the paragrahs and most content besides headers
    ...
    if (text.isEmpty) {
      ...
      if (children.isEmpty) {
        return Padding(
          ...
          child: Text(text, style: const TextStyle(fontFamily: 'Avenir')),
        );
      } else {
        return RichText(
          text: TextSpan(
            ...
            children: [
              TextSpan(text: text),
              ...children.map((child) {
                if (child is MarkdownLinkWidget) {
                  return TextSpan(
                    text: child.text,
                    ...
                    recognizer: TapGestureRecognizer()
                      ..onTap = () => launchUrl(Uri.parse(child.href)),
                  );
                }
    ...
    if (p.extension(path) == ".svg") {
      return Flexible(
          ...
          child: SvgPicture.asset(
            ...
            semanticsLabel: widget.alt.isNotEmpty ? widget.alt : null,
          ));
    } else {
      return Flexible(
          fit: FlexFit.tight,
          ...
          child: Image.asset(
            path,
            fit: BoxFit.scaleDown,
            semanticLabel: widget.alt.isNotEmpty ? widget.alt : null,
          ));
    }
  }
}
...
abstract class MarkdownListWidget extends StatelessWidget {
  final List<Widget> children;

  const MarkdownListWidget({super.key, required this.children});

  @override
  Widget build(BuildContext context) {
    final olChildren = List<Widget>.generate(
      children.length,
      (index) {
        final child = children[index];
        // Handle nested lists by not adding bullets
        if (child is MarkdownOrderedWidget ||
            child is MarkdownUnorderedWidget) {
          return Padding(
            padding: const EdgeInsets.only(left: 20),
            child: child,
          );
        }
        // Regular list items get bullets
        return Row(
            crossAxisAlignment: CrossAxisAlignment.start,
            children: buildChild(index, child));
      },
    );
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
  ...
  List<Widget> buildChild(int index, Widget child);
}

class MarkdownUnorderedWidget extends MarkdownListWidget {
  const MarkdownUnorderedWidget({super.key, required super.children});

  @override
  List<Widget> buildChild(int index, Widget child) {
    return [
      const Text('• ', style: TextStyle(fontFamily: 'Avenir')),
      Expanded(child: child),
    ];
  }
}

class MarkdownOrderedWidget extends MarkdownListWidget {
  const MarkdownOrderedWidget({super.key, required super.children});

  @override
  List<Widget> buildChild(int index, Widget child) {
    return [
      Text('${index + 1}. ', style: const TextStyle(fontFamily: 'Avenir')),
      Expanded(child: child),
    ];
  }
}
...
class _OrderedBuilder extends Builder<MarkdownOrderedWidget, Widget> {
  @override
  MarkdownOrderedWidget build() {
    return MarkdownOrderedWidget(children: children());
  }
}
