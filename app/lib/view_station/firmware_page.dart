...
class StationFirmwarePage extends StatelessWidget {
  final ColorFilter colorFilter =
      const ColorFilter.mode(Color(0xff2d3e4f), BlendMode.srcIn);
  ...
  Widget _buildStationCard(
    ...
    final firmwareReleaseDate = _formatFirmwareReleaseDate(
        DateTime.fromMillisecondsSinceEpoch(
            config.firmware.time.toInt() * 1000));
    return Card(
        ...
        child: ListTile(
            ...
            leading: SizedBox(
              ...
              child: station.connected
                  ? SvgPicture.asset(AppIcons.stationConnected)
                  : SvgPicture.asset(
            ...
            subtitle: Column(children: [
              ...
              Align(
                  alignment: Alignment.topLeft,
                  child: Text(
                      localizations.firmwareReleased(firmwareReleaseDate),
                      style: const TextStyle(fontSize: 14))),
              Align(
                  alignment: Alignment.topLeft,
                  child: Text(
  ...
  Widget _buildFirmwareUpdateCard(
    ...
    for (final firmware in availableFirmware.firmware) {
      final time = DateTime.fromMillisecondsSinceEpoch(firmware.time.toInt());
      Loggers.ui.t(
      ...
      if (FirmwareComparison.compare(firmware, config.firmware).newer) {
        final firmwareReleaseDate = _formatFirmwareReleaseDate(time);
        return Card(
          shadowColor: Colors.white,
          child: Container(
            color: Colors.white,
            ...
            child: ListTile(
                tileColor: Colors.white,
                title: Text(localizations.firmwareNotUpdated),
                subtitle:
    ...
    return Card(
        ...
        child: Container(
          ...
          child: ListTile(
            tileColor: Colors.white,
            title: Text(localizations.firmwareUpdated),
          ),
        ));
  }
  ...
  String _formatFirmwareReleaseDate(DateTime time) {
      ...
      if (comparison.newer) {
        return GenericListItemHeader(
            ...
            subtitle: AppLocalizations.of(context)!
                .firmwareReleased(formatter.format(comparison.localTime)));
      } else {
        return GenericListItemHeader(
          title: AppLocalizations.of(context)!.firmwareVersion(title),
          subtitle: AppLocalizations.of(context)!
        ...
        children: [
          ...
          Semantics(
            label: comparison.newer
                ? '${localizations.firmwareUpgrade} ${localizations.firmwareVersion(title)}'
                : '${localizations.firmwareSwitch} ${localizations.to} ${localizations.firmwareVersion(title)}',
            child: ElevatedTextButton(
...
class FirmwareBranch extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      ExpandableItems(
          heading:
              Text(AppLocalizations.of(context)!.moreItems(items.length - 1)),
