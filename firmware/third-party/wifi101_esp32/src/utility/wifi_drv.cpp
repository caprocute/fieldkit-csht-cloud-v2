...
int8_t WiFiDrv::disconnect() {
    ...
    uint8_t _data = 0;
    uint8_t _dataLen = 0;
    result = SpiDrv::waitResponseCmd(DISCONNECT_CMD, PARAM_NUMS_1, &_data, &_dataLen);
...
uint8_t WiFiDrv::reqHostByName(const char *aHostname) {
    ...
    // Wait for reply
    uint8_t _dataLen = 0;
    result = SpiDrv::waitResponseCmd(REQ_HOST_BY_NAME_CMD, PARAM_NUMS_1, &_data, &_dataLen);
...
int8_t WiFiDrv::wifiSetEnterprise(uint8_t eapType, const char *ssid, uint8_t ssid_len, const char *username, const uint8_t username_len,
                                  const char *password, const uint8_t password_len, const char *identity, const uint8_t identity_len,
...
int8_t WiFiDrv::fileOperation(uint8_t operation, const char *filename, uint8_t filename_len, uint32_t offset, uint8_t *buffer,
                              uint32_t len) {
    uint8_t _data = 0;
    uint8_t _dataLen = 0;
