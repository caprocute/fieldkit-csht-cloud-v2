...
static uint8_t SLAVERESET = 96u;  // reset pin
...
void SpiDrv::spiSlaveSelect()
{
    ...
    /**
    I used the following code to gauge how many iterations we need, this is
    close and probably a little longer than the original 5ms.
    while (true) {
        uint32_t started = fk_uptime();
        uint32_t counter = 0;
        while (fk_uptime() - started < 5) {
            counter++;
        }
    ...
    */
    uint32_t delay = 16384;
    for (; (digitalRead(SLAVEREADY) != HIGH) && delay > 0;) {
        delay--;
    }
}
