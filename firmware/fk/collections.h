...
public:
    ...
    T *get(size_t index) {
        for (auto iter = head_; iter != nullptr; iter = iter->np, index--) {
    ...
    template <typename SortKeyFn> bool sort(SortKeyFn key_fn) {
        auto modified = false;
        if (head_ == nullptr) {
            return modified;
        }
...
template <typename K, size_t TableSize = 10> struct hash_key {
    uint32_t operator()(K const &key) const {
        return reinterpret_cast<uint32_t>(key) % TableSize;
    }
};

template <typename K, typename V> class hash_node {
private:
    K key_;
    V value_;
    hash_node *np_;
 ...
 */
template <typename K, typename V, size_t TableSize = 10, typename F = hash_key<K, TableSize>> class hash_map {
private:
    Pool *pool_{ nullptr };
