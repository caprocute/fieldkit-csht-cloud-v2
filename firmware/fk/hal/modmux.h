...
public:
    ...
    bool virt() const {
        return integer_ == Virtual.integer();
    }
...
public:
    ...
    virtual bool try_read_eeprom(uint32_t address, uint8_t *data, size_t size) = 0;
    ...
    virtual void get_power_status(char *buffer, size_t size) {
    }
    virtual ModulePower get_module_power(ModulePosition position) {
        return ModulePower::Unknown;
    }
