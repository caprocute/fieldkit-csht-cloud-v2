...
static bool ina219_write(TwoWireWrapper &bus, uint8_t address, uint8_t reg, uint16_t value) {
    uint8_t data[] = {
        reg,
        (uint8_t)((value >> 8) & 0xff),
        (uint8_t)((value)&0xff),
    };
    if (!I2C_CHECK(bus.write(address, data, sizeof(data)))) {
        return false;
    }
...
static bool ina219_read(TwoWireWrapper &bus, uint8_t address, uint8_t reg, uint16_t &value) {
    uint8_t data[2];

    if (!I2C_CHECK(bus.read_register_buffer(address, reg, data, sizeof(data)))) {
        return false;
    }
...
bool Ina219::begin(bool trigger) {
    ...
    config_ = INA219_CONFIG_BVOLTAGERANGE_16V | INA219_CONFIG_GAIN_1_40MV | INA219_CONFIG_BADCRES_12BIT |
              INA219_CONFIG_SADCRES_12BIT_1S_532US | (trigger ? INA219_CONFIG_MODE_SANDBVOLT_TRIGGERED : INA219_CONFIG_MODE_POWERDOWN);

    // 32V 1A
    calibration_value_ = 10240;
    ma_divider_ = 25.0f;
    power_multiplier_ = 0.8f;
    config_ = INA219_CONFIG_BVOLTAGERANGE_32V | INA219_CONFIG_GAIN_8_320MV | INA219_CONFIG_BADCRES_12BIT |
              INA219_CONFIG_SADCRES_12BIT_1S_532US | (trigger ? INA219_CONFIG_MODE_SANDBVOLT_TRIGGERED : INA219_CONFIG_MODE_POWERDOWN);
...
MeterReading Ina219::read() {
    ...
    if (!ina219_read(*bus_, address_, INA219_REGISTER_VOLTAGE_BUS, value)) {
        return { .available = false };
    }
    ...
    if (!ina219_read(*bus_, address_, INA219_REGISTER_VOLTAGE_SHUNT, value)) {
        return { .available = false };
    }
    auto shunted_voltage_raw = (int32_t)(int16_t)value;

    if (!ina219_read(*bus_, address_, INA219_REGISTER_CURRENT, value)) {
        return { .available = false };
    }
    auto ma_raw = (int32_t)(int16_t)value;

    if (!ina219_read(*bus_, address_, INA219_REGISTER_POWER, value)) {
        return { .available = false };
    }
