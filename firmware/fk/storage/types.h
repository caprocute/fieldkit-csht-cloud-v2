...
struct SeekSettings {
    ...
    SeekSettings(FileNumber file, RecordNumber record) : file(file), record(record) {
    }
...
struct SeekValue {
    ...
    SeekValue(StorageAddress address, RecordNumber record, StorageSize position, BlockNumber block, StorageTime timestamp,
              StorageAddress record_address, StorageSize bytes_in_block, RecordNumber records_in_block)
        : address(address), record(record), position(position), block(block), timestamp(timestamp), record_address(record_address),
          bytes_in_block(bytes_in_block), records_in_block(records_in_block) {
    }
