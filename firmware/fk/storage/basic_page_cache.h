...
template <typename PageStoreType, size_t PageSize, size_t N> class BasicPageCache : public PageCache {
private:
...
public:
    ...
    bool flush(CachedPage *page) override {
        ...
        logdebug("[0x%06" PRIx32 "]: flush #%" PRIu32 " dirty (0x%3x - 0x%3x) (0x%06" PRIx32 " - 0x%06" PRIx32 ") %" PRIu32 " bytes",
                 page_address, (uint32_t)page->page, page->dirty_start, page->dirty_end, page_address + page->dirty_start,
                 page_address + page->dirty_end, page->dirty_end - page->dirty_start);

        if (!store_.save_page(page_address, page->ptr, PageSize, page->dirty_start, page->dirty_end)) {
            logerror("[0x%06" PRIx32 "]: flush #%" PRIu32 " dirty (0x%3x - 0x%3x) (0x%06" PRIx32 " - 0x%06" PRIx32 ") %" PRIu32
                     " bytes failed!",
                     page_address, (uint32_t)page->page, page->dirty_start, page->dirty_end, page_address + page->dirty_start,
                     page_address + page->dirty_end, page->dirty_end - page->dirty_start);
            return false;
