...
public:
    ...
    void show_set_time() override {
        set_time_view.prepare();
        show_view(set_time_view);
    }
    ...
    void run(DisplayTaskParameters *params) {
        if (!leds.begin()) {
        ...
        while (!can_stop || !stop_timer.expired()) {
            ...
            if (get_ipc()->dequeue_button(&button, DisplayButtonWaitMs)) {
                // If we were woken by a button press, lifting "up" will send
                // the button event and that can come in just as we've started.
                // We ignore that by ignoring button presses before the display
                // task was started.
                if (button->after(started)) {
                    stop_timer.mark();
            ...
            if (frame_pool->used() > 0) {
                if (frame_pool->used() > maximum_used) {
                    ...
                    loginfo("display-pool: %zu", maximum_used);
                }
                frame_pool->clear();
            }
...
void task_handler_display(void *params) {
    ...
    views.run((DisplayTaskParameters *)params);
    get_module_leds()->off();
    get_display()->off();
}
