...
void SelfCheck::check(SelfCheckSettings settings, SelfCheckCallbacks &callbacks, Pool *pool) {
    ...
    status_ = SelfCheckStatus{};

    status_.started = fk_uptime();

    callbacks.update(status_);

    status_.rtc.status = to_status(rtc());
    callbacks.update(status_);

    if (settings.check_battery) {
        status_.battery_gauge.status = to_status(battery_gauge());
        status_.solar_gauge.status = to_status(solar_gauge());
    } else {
        status_.battery_gauge.status = CheckStatus::Unknown;
        status_.solar_gauge.status = CheckStatus::Unknown;
    }
    callbacks.update(status_);

    if (settings.check_temperature) {
        status_.temperature.status = to_status(temperature());
    } else {
        status_.temperature.status = CheckStatus::Unknown;
    }
    callbacks.update(status_);

    status_.qspi_memory.status = to_status(qspi_memory());
    callbacks.update(status_);

    status_.spi_memory.status = to_status(spi_memory());
    callbacks.update(status_);

    if (settings.check_network) {
        status_.wifi.status = to_status(wifi(pool));
    } else {
        status_.wifi.status = CheckStatus::Unknown;
    }
    callbacks.update(status_);

    if (settings.check_gps) {
        status_.gps.status = to_status(gps());
    } else {
        status_.gps.status = CheckStatus::Unknown;
    }
    callbacks.update(status_);

    if (settings.check_sd_card) {
        status_.sd_card_open.status = to_status(sd_card_open());
        status_.sd_card_write.status = to_status(sd_card_write());
    } else {
        status_.sd_card_open.status = CheckStatus::Unknown;
        status_.sd_card_write.status = CheckStatus::Unknown;
    }
    callbacks.update(status_);

    if (settings.check_backplane) {
        status_.bp_shift.status = to_status(backplane_shift());
        callbacks.update(status_);

        status_.bp_mux.status = to_status(backplane_mux());
        callbacks.update(status_);

        status_.bp_leds.status = to_status(backplane_leds());
        callbacks.update(status_);
    } else {
        status_.bp_mux.status = CheckStatus::Unknown;
        status_.bp_shift.status = CheckStatus::Unknown;
        status_.bp_leds.status = CheckStatus::Unknown;
    }

    if (settings.module_presence) {
        status_.modules = modules(pool);
    }

    if (settings.check_lora) {
        status_.lora.status = to_status(lora());
        callbacks.update(status_);
    }
...
void SelfCheck::save() {
    ...
    gs.get()->general.self_check = status_;
}
...
ModuleCheckStatus SelfCheck::modules(Pool *pool) {
    return single_noncritical_check<ModuleCheckStatus>("modules", [=]() {
        logwarn("module-check: start");
