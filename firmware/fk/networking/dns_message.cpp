...
BufferPtr *DNSWriter::query_service_type(uint16_t xid, const char *service_type) {
    header()->xid = xid;
...
BufferPtr *DNSWriter::answer_service_type(uint16_t xid, const char *service_type, const char *name) {
...
BufferPtr *DNSWriter::finish() {
    return pool_->wrap(buffer_, size_, writer_.position());
}
