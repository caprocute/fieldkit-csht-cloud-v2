...
void LoraWorker::run(Pool &pool) {
    ...
    switch (lora.outcome()) {
    case LoraOutcome::None: {
        break;
    }
    case LoraOutcome::JoinOk: {
        LoraEvent event = LoraEvent::joined();
        AppendEventWorker append{ &event };
        append.run(pool);
        break;
    }
    case LoraOutcome::JoinFailed: {
        LoraEvent event = LoraEvent::failed_join();
        AppendEventWorker append{ &event };
        append.run(pool);
        break;
    }
    case LoraOutcome::ConfirmedSendOk: {
        LoraEvent event = LoraEvent::confirmed();
        AppendEventWorker append{ &event };
        append.run(pool);
        break;
    }
...
bool LoraWorker::verify(LoraManager &lora, Pool &pool) {
    loginfo("verify");

    // This is basically an order to force join.
    if (!lora.force_join(pool)) {
        return false;
    }
...
bool LoraWorker::packets(LoraManager &lora, uint8_t port, BufferPtr *iterator, Pool &pool) {
    ...
    while (iterator != nullptr) {
        ...
        if (!lora.send_bytes(port, iterator->buffer(), iterator->position(), pool)) {
            return false;
        }
