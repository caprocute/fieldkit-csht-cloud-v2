...
EnableModulePower::~EnableModulePower() {
    if (position_ == ModulePosition::Virtual) {
        return;
    }

    auto mm = get_modmux();
    auto override = mm->get_module_power(position_);

    if (enabled_once() || override == ModulePower::PowerCycle) {
        if (!mm->disable_module(position_)) {
...
bool EnableModulePower::enable() {
    ...
    if (enabled_once() || always_enabled()) {
        auto enabling = !mm->is_module_on(position_);
        if (!enabling) {
