...
bool ModuleScanning::try_scan_single_module(ScanningListener *listener, ModulePosition position, Pool &pool) {
    ModuleHeader header;
    bzero(&header, sizeof(ModuleHeader));
    if (!mm_->try_read_eeprom(ModuleEeprom::HeaderAddress, (uint8_t *)&header, sizeof(ModuleHeader))) {
    ...
    if (!fk_module_header_valid(&header)) {
        ...
        fk_dump_memory("hdr ", (const uint8_t *)&header, sizeof(ModuleHeader));
...
bool ModuleScanning::provision(ModulePosition position, ModuleHeader &header) {
    ...
    if (!eeprom) {
        logwarn("[%] no eeprom", position.integer());
        return false;
    }
    ...
    if (rewrite) {
        loginfo("[%d] overwriting invalid header", position.integer());
