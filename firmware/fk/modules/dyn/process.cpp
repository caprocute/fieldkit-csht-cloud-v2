...
uint32_t allocate_process_got(fkb_header_t const *header, uint32_t *got, uint32_t *data) {
    ...
    loginfo("[0x%8p] number-syms=%" PRIu32 " number-rels=%" PRIu32 "got=0x%" PRIx32, base, header->number_symbols,
            header->number_relocations, header->firmware.got_offset);
    ...
    for (auto i = 0u; i < header->number_symbols; ++i) {
        ...
        loginfo("[0x%8p] %-6s addr=0x%8" PRIx32 " size=0x%4" PRIx32 " '%s' (0x%8p = 0x%" PRIx32 ")", base, linked ? "linked" : "alloc",
                sym->address, sym->size, sym->name, ptr, *ptr);
...
public:
    template <typename R> optional<R> walk(std::function<optional<R>(fkb_header_t const *)> fn) {
        auto ptr = begin();
...
void Process::run(Pool &pool) {
    ...
    if (false) {
        ...
        if (!copy_memory_to_flash((uint8_t *)running, running->firmware.binary_size, &flash, FK_MEMORY_QSPI_ADDRESS_UPGRADE_CORE, 4096,
                                  pool)) {
            logerror("error copying binary");
            return;
        }

        if (!copy_memory_to_flash((uint8_t *)running, running->firmware.binary_size, &flash, FK_MEMORY_QSPI_ADDRESS_FAILSAFE_CORE, 4096,
                                  pool)) {
            logerror("error copying binary");
            return;
        }
    ...
    if (false) {
        if (!copy_memory_to_flash(build_samd51_modules_dynamic_main_fkdynamic_fkb_bin,
                                  build_samd51_modules_dynamic_main_fkdynamic_fkb_bin_len, &flash, 512 * 1024, 4096, pool)) {
            logerror("error copying binary");
            return;
        }
    }
