...
template <typename T> class PoolPointer {
public:
    virtual ~PoolPointer() {
    }
...
public:
    ...
    template <typename K> K *get() {
        return reinterpret_cast<K *>(get());
    }
};

template <typename Wrapped, typename ConcreteWrapped = Wrapped, class... Args> class WeakPoolWrapper : public PoolPointer<Wrapped> {
private:
    Pool *pool_;
...
template <typename Wrapped, typename ConcreteWrapped = Wrapped, class... Args> class StandardPoolWrapper : public PoolPointer<Wrapped> {
private:
    Pool *pool_;
    ConcreteWrapped wrapped_;
...
template <typename Wrapped, typename Wrapee = PoolPointer<Wrapped>, typename ConcreteWrapped = Wrapped,
          typename ConcreteWrapee = StandardPoolWrapper<Wrapped, ConcreteWrapped>, class... Args>
