...
#define EEPROM_I2C_ADDRESS_JIG (0x51)
...
#else // FK_ENABLE_SHT31_AND_MPL3115A2
        // failures |= FK_WEATHER_SENSORS_SHT31;
        // failures |= FK_WEATHER_SENSORS_MPL3115A2;
#endif // FK_ENABLE_SHT31_AND_MPL3115A2
    }
...
__int32_t main() {
    ...
    // TODO: This should be a compile time assertion.
    ...
    while (true) {
        ...
        uint8_t discarding[4];
        if (eeprom_read_page(&I2C_1, EEPROM_I2C_ADDRESS_JIG, 0, discarding, sizeof(discarding)) != FK_SUCCESS) {
            registers[1] = FK_WEATHER_JIG_FAIL;
        } else {
            registers[1] = FK_WEATHER_JIG_PASS;
        }
    ...
    if (rv != FK_SUCCESS) {
        if (sensors.initialized == 0) {
            ...
            delay_ms(8000);
            NVIC_SystemReset();
#else // FK_WEATHER_IGNORE_NO_SENSORS
            loginfo("ignoring no sensors");
#endif // FK_WEATHER_IGNORE_NO_SENSORS
        }
    }
