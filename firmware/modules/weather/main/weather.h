...
#define FK_WEATHER_SENSORS_ALL                                                                                                             \
    FK_WEATHER_SENSORS_COUNTERS | FK_WEATHER_SENSORS_ADC | FK_WEATHER_SENSORS_SHT31 | FK_WEATHER_SENSORS_MPL3115A2 |                       \
        FK_WEATHER_SENSORS_BME280;

static inline float fkw_weather_temperature_1(fk_weather_aggregated_t *w) {
    return ((float)(int32_t)w->temperature_1) / 100.0f;
}
