    ...
    WindReading get_wind_30_second_average(Pool &pool) {
        ...
        for (auto i = 0u; i < 30; ++i) {
            if (aw->wind_120s[index].ticks != FK_WEATHER_TICKS_NULL) {
                logverbose("wind120s[%d/%d] %" PRIu32 " %" PRIu32, i, index, aw->wind_120s[index].ticks, aw->wind_120s[index].direction);
...
bool AggregatedWeather::try_initialize(ModuleContext mc, Pool &pool) {
    ...
    if (aw->initialized) {
        if (!(aw->failures == FK_WEATHER_SENSORS_BME280 || aw->failures == (FK_WEATHER_SENSORS_SHT31 | FK_WEATHER_SENSORS_MPL3115A2))) {
            logwarn("sensor module failures");
            return false;
        }
    } else {
...
ModuleReadings *AggregatedWeather::take_readings(ModuleContext mc, Pool &pool) {
    ...
    // If the number of failures changed on the module, something has gone
    // wrong. We only pay attention to the failures if the data that's come back
    // is good.
    if (aw->initialized == 0 && aw->failures == 0) {
        logerror("module still initializing");
    } else {
        auto failures_changed = failures_ >= 0 && failures_ != aw->failures;
        failures_ = (int8_t)aw->failures;
        if (failures_changed) {
            logerror("new failures (%d != %d), hup module", failures_, aw->failures);
            return nullptr;
        }
    }
    ...
    if (having_supply_chain_nightmare) {
        ...
        int32_t temp_2 = (int32_t)aw->temperature_2;
        ...
        mr->set(i++, SensorReading{ now, temp_2 / 16.0f });
    ...
    collector_.include(aw);

    uint32_t total = collector_.ticks();
    int32_t rain_delta = total - last_rain_;
    last_rain_ = total;

    mr->set(i++, SensorReading{ now, (float)rain_delta, rain_delta * RainPerTick });
