...
TEST_F(DnsSuite, BuildQueryServiceType) {
    ...
    ASSERT_EQ(encoded->position(), sizeof(packet_16));
    ASSERT_EQ(encoded->position(), 38u);
    ASSERT_TRUE(0 == std::memcmp(encoded->buffer(), packet_16, encoded->position()));

    auto buffer = encoded->take();
    DNSReader message{ &pool, buffer, encoded->position() };
    message.parse();
}
...
TEST_F(DnsSuite, BuildAnswerService) {
    ...
    DNSReader message{ &pool, encoded->take(), encoded->position() };
    message.parse();
