...
TEST_F(HttpBasicParsingSuite, SimpleGet2) {
    ...
    const char *req_header = "GET /fk/1/status HTTP/1.1\n"
                             "Connection: close\n"
                             "Content-Length: 32\n"
                             "Content-Type: text/plain\n"
...
TEST_F(HttpRoutingSuite, WithMultipleBasicRoutes) {
    ...
    HttpRoute routes[4]{ { "/fk/v1", &handlers[0] }, { "/fk/v2", &handlers[1] }, { "/fk/v3", &handlers[2] }, { "/fk/v4", &handlers[3] } };
...
TEST_F(HttpParsingQuerySuite, SimpleGet1) {
    ...
    auto req_header = pool_.sprintf("GET / HTTP/1.1\n"
                                    "Content-Type: text/plain\n"
                                    "Content-Length: %d\n"
                                    "\n",
                                    encoded->position());
    ...
    ASSERT_EQ(req.parse(req_header, strlen(req_header)), 59);
    ASSERT_EQ(req.parse((const char *)encoded->buffer(), encoded->position()), 3);
