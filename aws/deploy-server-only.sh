...
# Sử dụng: ./deployment/deploy-server-only.sh [VERSION] [ENVIRONMENT]
# Ví dụ: ./deployment/deploy-server-only.sh latest staging
...
echo "=========================================="
echo "Build và Deploy Server Service (No Migration)"
echo "=========================================="
...
echo "📦 Bước 1: Build và push Docker image (chỉ server)..."
echo ""

if ! ./deployment/build-and-push-server.sh ${VERSION} ${ENVIRONMENT}; then
...
echo "  - FIELDKIT_MAPBOX_TOKEN: đã được cập nhật"
echo ""
