...
Ads1219::Ads1219(uint8_t address, Ads1219ReadyChecker *ready)
    : address_(address), ready_(ready), config_(0) {}

Ads1219::~Ads1219() {}

bool Ads1219::begin() {
  if (!I2C_CHECK(wire_write_u8(address_, ADS1219_COMMAND_RESET))) {
    return false;
  }
...
bool Ads1219::configure(Ads1219VoltageReference vref, Ads1219Channel channel,
                        Ads1219Gain gain, Ads1219DataRate dr) {
  config_ = (uint8_t)vref | (uint8_t)channel | (uint8_t)gain | (uint8_t)dr;

  // logdebug("config=0x%x", config_);
  if (!I2C_CHECK(wire_write_register_u8(
          address_, ADS1219_COMMAND_WRITE_CONFIG_REGISTER, config_))) {
    return false;
  }

  uint8_t config_after = 0;
  if (!I2C_CHECK(wire_read_register_u8(
          address_, ADS1219_COMMAND_READ_CONFIG_REGISTER, config_after))) {
    return false;
  }
  ...
  if (!I2C_CHECK(wire_read_register_u8(
          address_, ADS1219_COMMAND_READ_STATUS_REGISTER, status_after))) {
    return false;
  }
...
bool Ads1219::read_conversion(int32_t &value) {
  ...
  if (!I2C_CHECK(wire_read_register_buffer(address_, ADS1219_COMMAND_READ_DATA,
                                           data, sizeof(data)))) {
    return false;
  }
...
bool Ads1219::start() {
  return I2C_CHECK(wire_write_u8(address_, ADS1219_COMMAND_START));
}
